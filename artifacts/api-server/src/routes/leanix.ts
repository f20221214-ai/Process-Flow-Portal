...
// ─── Route: seed demo initiatives (local dev) ────────────────────────────────

router.post("/leanix/seed-demo", async (req, res) => {
  try {
    const now = new Date();
    let added = 0;
    let updated = 0;
    ...
    res.json({ success: true, added, updated, total: DEMO_LEANIX_INITIATIVES.length });
  } catch (err) {
    req.log.error({ err }, "Failed to seed demo LeanIX initiatives");
    res.status(500).json({ error: "Failed to seed demo initiatives." });
  }
});
