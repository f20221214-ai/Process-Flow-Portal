...
- REGULATORY: none=no external compliance obligations, no new reporting or audits, single jurisdiction, negligible non-compliance consequence, well-established capability, no licences required, no cross-border transfers, stable requirements; low=internal policies only, minor new controls, single country, low consequence, mostly established capability, existing licences cover it; medium=industry standards/certifications, some new external obligations, multiple regions with comparable rules, moderate consequence (reputational/contractual), partial compliance capability, licence variation needed, adequacy-covered cross-border transfers, reactive change management; high=government legislation (privacy, financial, health, food safety) OR legal/financial penalties for non-compliance OR multiple jurisdictions with conflicting rules OR no compliance capability OR new licence/government approval required OR cross-border transfers with no established legal mechanism (SCCs/BCRs not in place) OR no regulatory change management process
